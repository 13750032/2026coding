...
#母音 Vowels: a,i,o,u。長度k的小字串,最多幾個母音
class Solution:
    def maxVowels(self, s: str, k: int) -> int:
        vowels = set('aeiou')#把5個字母,變成set()
        #以後用ifcinvowels:就可以快速確認它是母音
        #以前 if co='a' or c-='e' or c=='i' or c=='o' or c-='u':
        count=0#記錄目前有幾個母音
        for i in range(k):#找前面 k個字母,逐一檢查,看是不是母音
            if s[i] in vowels: count+=1#找到1個母音,開心!!!
        ans=count #離開廻圈時,確認前k個字母,有 count 個母音,先當答案
        N=len(s)#全部字串的長度 N
